...
```{r read-recipes}
cupcakes_html <- cupcake_sample %>% map(read_html_slow)
muffin_html <- muffin_sample %>% map(read_html_slow)
beep(4)
```
...
fruits <- c("\\w*apple", "\\w*berr(y|ies)", "raisin", "lemon", "banna",
