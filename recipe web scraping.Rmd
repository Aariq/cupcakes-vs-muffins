...
```{r}
cupcakes_df2 <- cupcakes_df %>% filter(str_detect(tolower(part), "cake"))
cupcakes_df2 %>% group_by(`Recipe ID`) %>% summarise(n()) %>% nrow()
```

```{r bind-all-recipes}
#add "cupcake" or "muffin" as name
recipes <- bind_rows(cupcakes_df2, muffins_df, .id = "Type") %>% mutate(Type = ifelse(Type == 1, "cupcake", "muffin"))
recipes
...
# Create wide data for multivarite analysis
```{r}
