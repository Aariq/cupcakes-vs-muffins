...
```{r}
recipes_tidy <- recipes_tidy %>% mutate(amount_per = amount_sum/servings)
```
...
nofrosting_tidy <- recipes5b %>%
  ...
  filter(Type == "muffin" | Type == "cupcake" & str_detect(tolower(part), "cake") & !str_detect(tolower(part), "decoration")) %>% 
