...
recipes2 %>% select(value, ingredient, units, amount, everything()) %>% filter(ingredient == "other")
```
...
# Filter out frosting
We all know cupcakes have frosting and muffins don't, so let's create a subset of `recipes_tidy` that removes frosting.
```{r}
nofrosting_tidy <- recipes5b %>%
  #all ingredients that are from muffin recipes or cupcake recipes where the part contains the word "cake"
  filter(Type == "muffin" | Type == "cupcake" & str_detect(tolower(part), "cake")) %>% 
...
```{r}
nofrosting_tidy <- nofrosting_tidy %>% select(-calories)
```

## Write to RDS
```{r}
write_rds(nofrosting_tidy, "nofrosting_tidy.rds")
```
...
# recipes_tidy %>% filter(is.na(amount_sum)) #this doesn't work because flour just isn't listed as an ingredient
